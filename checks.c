...
/*   Updated: 2025/03/22 13:20:11 by aumoreno         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_map_is_valid(t_game *game, char *joined_str)
{
    ...
    while(joined_str[++i] != 0)
    {
        if(joined_str[i] == '\n')
            continue;
        /*CHECK IF MAP IS FULLY SURROUNDED BY WALLS*/
        if(ft_issurrounded(game, i))
        {
            // de momento hasta el innit map, game no tiene nada alojado mas que él mismo, no tiene ni img ni nada 
            if(joined_str[i] != '1')
